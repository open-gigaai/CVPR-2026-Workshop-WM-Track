...
# curobo 是可选依赖；planner.py 在未安装时不会定义 CuroboPlanner
try:
    from .planner import CuroboPlanner  # type: ignore
except Exception:  # pragma: no cover
    CuroboPlanner = None  # type: ignore
...
class Robot:
    ...
    def reset(self, scene, need_topp=False, **kwargs):
        ...
        if self.communication_flag:
            ...
            # curobo 未安装时，CuroboPlanner 为 None；此时只要使用 mplib planner 即可
    ...
    def set_planner(self, scene=None):
        ...
        # 如果未安装 curobo，则强制走非通信模式的 mplib planner
        if CuroboPlanner is None:
            self.communication_flag = False

        if not self.communication_flag:
            if CuroboPlanner is None:
                # 回退：用 MplibPlanner 提供 plan_grippers（以及可能的 plan_path）
