...
class Base_Task(gym.Env):
    ...
    def get_obs(self):
        ...
        if self.data_type.get("pointcloud", False):
            ...
            pkl_dic["world_pcd"] = self.cameras.get_world_pcd()
    ...
    def take_action(self, action, action_type='qpos'):  # action_type: qpos or ee
        if self.take_action_cnt == self.step_lim:
            return
        # print(self.now_obs["observation"]["head_camera"]["rgb"].shape)
        eval_video_freq = 1  # fixed
        # if (self.eval_video_path is not None and self.take_action_cnt % eval_video_freq == 0):
            # self.eval_video_ffmpeg.stdin.write(self.now_obs["observation"]["head_camera"]["rgb"].tobytes())
            # imageio.imwrite(self.eval_video_path / f"{self.take_action_cnt:03d}.png",
            #                 self.now_obs["observation"]["head_camera"]["rgb"])
            # print('save at ', self.eval_video_path / f"{self.take_action_cnt:03d}.png")
